...
public class paqueteDao extends DAO {
    ...
    public List<paquete> listarPaqueteDuo() throws Exception {
        ...
        try {
            this.conectar();
            query = "select nombre, descripcion, precioventa from paquetes where id_tiposervicio=3";
    ...
    public List<paquete> listarPaqueteInternet() {
        ...
        try {
            ...
            this.conectar();
            query = "select nombre, descripcion, precioventa from paquetes where id_tiposervicio=2";
            sta = this.getCn().prepareStatement(query);
            res = sta.executeQuery();
            lis = new ArrayList();
    ...
    public List<paquete> listarPaqueteCable() {
        ...
        try {
            this.conectar();
            query = "select nombre, descripcion, precioventa from paquetes where id_tiposervicio=1";
            sta = this.getCn().prepareStatement(query);
            res = sta.executeQuery();
            lis = new ArrayList();
